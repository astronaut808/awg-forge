...
## Лабораторный профиль AWG 3.0

AWG 3.0 не входит в стабильный образ и сценарий установки. Это opt-in лабораторный профиль, пока upstream userspace, kernel и клиентская поддержка продолжают меняться. AWG 2.0 остается профилем по умолчанию и поддерживаемым production-вариантом.

Lab-сборка использует закрепленные upstream-коммиты, принудительно запускает AWG 3.0 через userspace `amneziawg-go` и показывает профиль только при одновременном наличии lab-образа и `AWG3_EXPERIMENTAL=true`. Обычный образ нельзя превратить в AWG3 одной переменной окружения.
...
Для тестов AWG 3.0 используй только скачанный `.conf`. AWG3 QR, `vpn://`, kernel-module runtime и заявления о production-совместимости намеренно не входят в этот режим. Перед использованием lab-туннеля обязательно проверь точную платформу и версию клиентского приложения.
...
`AWG3_EXPERIMENTAL` по умолчанию имеет значение `false`. Она действует только в собранном локально AWG3 lab-образе; установка переменной в обычном образе не откроет профиль.
