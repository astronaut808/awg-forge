...
AWG 3.x получает явный `1280` только при создании нового туннеля, когда исходное значение настройки равно `0`. Это запасное значение совместимости, выбранное в awg-forge, а не требование протокола AWG. `1280` является минимальным MTU канала для IPv6 и одновременно текущим значением по умолчанию в клиентах Amnezia для Android, iOS и macOS Network Extension. Настольные сборки пока используют `1376`, а upstream PR #3065 только предлагает унифицировать запасное значение на `1280`. awg-forge сейчас создает клиентские маршруты только для IPv4, поэтому минимум IPv6 здесь служит консервативным инженерным ориентиром, а не протокольной гарантией. Меньшее значение также оставляет больше места для накладных данных экспериментального профиля и не зависит от платформенного значения клиента.
...
Если у туннеля явно задан MTU, awg-forge сохраняет его в серверном конфиге, скачиваемом клиентском `.conf`, структурированном QR для AmneziaVPN и raw-config экспорте `vpn://`. Новый туннель AWG 3.x использует `1280` как консервативный fallback, если MTU не задан; явные значения и существующее состояние туннелей сохраняются. Предварительный AmneziaVPN `5.0.2.0` все еще может заменить импортированное значение платформенным default. Исправление предложено в upstream PR #3113, но еще не выпущено, поэтому при рабочем handshake и зависании крупных пакетов проверь эффективный MTU на обеих сторонах.
