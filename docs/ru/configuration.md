...
Образ закрепляет `amneziawg-go` 3.1.20260828 и `amneziawg-tools` 3.1.20260812, принудительно запускает AWG 3.x через userspace и показывает профиль `awg_3` без отдельной переменной окружения, образа или Compose override. Версии до `amneziawg-go` 3.1.20260814 не поддерживаются: cookie reply с `RandomTrailers` мог завершить процесс. Закрепленный runtime 3.1.20260828 также содержит текущие upstream-исправления UDP-window padding и поведения `DisableCookies` под нагрузкой. Профиль переиспользует тот же генератор QUIC Initial-like `I1`, что и AWG 2.0.

Используй скачанный `.conf` или QR для AmneziaWG, который содержит тот же raw `.conf`. QR для AmneziaVPN и `vpn://` остаются выключенными до отдельной проверки этих форматов. Минимальная поддерживаемая версия AmneziaVPN для AWG 3.1 — 5.0.1.5; версия 5.0.0.5 несовместима.

Сгенерированные параметры Header Protection используют рекомендуемые upstream значения совместимости `H1=1`, `H2=2`, `H3=3` и `H4=4`, а каждое значение `S1-S4` не меньше `12`. `RandomTrailers` и `DisableCookies` по умолчанию имеют значение `off`. Серверный runtime config сохраняет явные значения `off`, чтобы live-обновление через `syncconf` сбрасывало ранее включенную опцию, а client export не содержит выключенные опции. Если `RandomTrailers` включается для контролируемого теста, upstream рекомендует одинаковые значения `S1-S4`; оставляй его выключенным, пока не решены открытые проблемы классификации transport-пакетов и первого пакета. `DisableCookies` отключает только отправку Cookie Reply: входящие cookie-сообщения и остальная cookie-логика WireGuard продолжают работать. Это все равно ослабляет защиту от handshake-нагрузки, поэтому вне контролируемого теста оставляй параметр выключенным.

Если у туннеля явно задан MTU, awg-forge записывает одинаковое значение в серверный и клиентский `.conf`. Для проверки совместимости AWG 3.x используй `1280` как консервативный fallback вместо `Авто`, когда path MTU неизвестен. Некоторые текущие пути импорта AmneziaVPN могут заменить MTU от сервера платформенным значением; upstream-исправление еще не принято, поэтому при рабочем handshake и зависании крупных пакетов проверь эффективный MTU на обеих сторонах.
