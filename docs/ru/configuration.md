...
Образ закрепляет `amneziawg-go` 3.1.20260814 и `amneziawg-tools` 3.1.20260812, принудительно запускает AWG 3.x через userspace и показывает профиль `awg_3` без отдельной переменной окружения, образа или Compose override. Более ранние релизы `amneziawg-go` 3.1 не поддерживаются: cookie reply с `RandomTrailers` мог завершить процесс. Профиль переиспользует тот же генератор QUIC Initial-like `I1`, что и AWG 2.0.
...
`RandomTrailers` и `DisableCookies` по умолчанию имеют значение `off`. Серверный runtime config сохраняет явные значения `off`, чтобы live-обновление через `syncconf` сбрасывало ранее включенную опцию, а client export не содержит выключенные опции. Оставляй `RandomTrailers` выключенным, пока upstream не исправит проблемы классификации transport-пакетов, первого пакета и MTU. Включение `DisableCookies` отключает WireGuard cookie replies и ослабляет защиту от handshake-нагрузки, поэтому вне контролируемого теста оставляй его выключенным.
