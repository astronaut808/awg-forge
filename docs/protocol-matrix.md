...
| `awg_3` | Laboratory only | One experimental AWG 3.x family profile, available only in the locally built lab image with explicit opt-in. It uses pinned `amneziawg-go` 3.1.20260814 and `amneziawg-tools` 3.1.20260812 userspace sources and renders `HeaderProtectionKey`, AWG3 ranges, `RandomTrailers`, and `DisableCookies`. Downloaded `.conf` and its raw AmneziaWG QR are supported; AmneziaVPN QR, `vpn://`, kernel runtime, and production compatibility are intentionally not supported. |
...
## AWG 3.x Laboratory Boundaries

The single AWG 3.x laboratory profile is derived from the current AmneziaVPN self-hosted generator and the pinned `amneziawg-go` 3.1.20260814 / `amneziawg-tools` 3.1.20260812 release revisions. It uses:
...
The numeric and range defaults mirror the upstream client generator. AWG-Forge intentionally differs for the security-sensitive toggles: upstream enables `RandomTrailers` and `DisableCookies`, while the laboratory profile keeps both off until endpoint compatibility is verified and cookie protection is explicitly waived. AWG3 config parsing in the pinned tools accepts the extended ranges as `uint16`; awg-forge rejects values above `65535` rather than allowing upstream truncation. `S1-S4` must be at least `12` when header protection is enabled.

AWG3 is forced through the pinned userspace runtime. `RandomTrailers` requires compatible 3.1 behavior on both endpoints. `DisableCookies=on` prevents cookie replies and weakens handshake-flood protection, so it must remain off outside controlled testing. Server configs retain explicit `off` values because runtime updates use `syncconf`; downloaded client configs omit disabled options, matching AmneziaVPN 5.0.1.5 canonical export. Use AmneziaVPN 5.0.1.5+ for interoperability tests and do not use 5.0.0.5 for AWG 3.1. Version 5.0.1.5 is a stable client release for Windows, Linux, macOS, and Android, but the AWG-Forge profile remains laboratory-only until a broader platform matrix and lifecycle coverage have been verified. The AmneziaWG QR contains the same verified raw `.conf` as the download. Do not enable AmneziaVPN QR, native JSON, or `vpn://` imports until those formats have been validated independently.

Manual end-to-end validation of the current lab image has confirmed `.conf` import, handshake, traffic, restart recovery, WAN egress, and WARP egress with compatible current AmneziaVPN, AmneziaWG, and DefaultVPN clients. This is a bounded laboratory acceptance result, not a guarantee for every platform, client build, network, or future 3.x runtime.
