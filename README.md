...
- Несколько поколений AmneziaWG: стабильные профили до 2.0 и изолированный opt-in lab-профиль AWG 3.x для раннего тестирования без изменения production-дефолта.
...
- AWG 3.x доступен только как экспериментальный lab-профиль в отдельном локально собранном образе с явным opt-in. Он не входит в стабильный образ или установщик и пока предназначен для проверки через скачанный `.conf` или QR для AmneziaWG, а не для production.
