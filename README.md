...
- AWG 3.x входит в стандартный образ как экспериментальный профиль с явным предупреждением в UI. Его включение и использование выполняются на свой риск; для production по умолчанию используется AmneziaWG 2.0. Проверенные способы импорта AWG 3.x — скачанный `.conf` и QR для AmneziaWG.
...
2. Открой `Config` у клиента и используй один из предложенных способов импорта. Для AWG 3.x доступны только скачивание `.conf` и QR для AmneziaWG.
